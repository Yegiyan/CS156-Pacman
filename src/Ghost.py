...
class Ghost:
    ...
    def update_position(self):
        ...
        if self.path and self.move_timer >= 1:
            next_step = self.path[0]  # Get the next step but do not pop it yet
            if self.is_wrap_around_move(self.grid_pos, next_step, max_cols):
                self.grid_pos = next_step  # Instantly update to the next position
                self.path.pop(0)  # Now remove the step from the path
    ...
    def find_new_path(self, grid, maze_module):
    ...
    def is_wrap_around_move(self, current_pos, next_pos, max_cols):
